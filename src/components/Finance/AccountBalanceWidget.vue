  ...
  <loading-container :isLoading="isLoading">
    <box
      ...
      <skeleton custom-class="h-9 w-48 rounded-xl mb-6 bg-white">
        <h2 class="font-bold text-4xl pb-4">{{ pesoFormatter.format(rawAccountBalance) }}</h2>
      </skeleton>
...
const { lastUpdated, isLoading, rawAccountBalance } = useFinancialRecordQuery(currentSemesterId!);
</script>